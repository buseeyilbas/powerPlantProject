...
# PER_BIN_MW: her 2-yıllık bin için SADECE o dönemde kurulan toplam güç (MW)
PER_BIN_MW = {}
...
try:
    if YEARLY_CHART_PATH.exists():
        ...
        for feat in chart.get("features", []):
            ...
            etype = props.get("energy_type")

            # step1_3'ün ürettiği heading_sub feature'larından oku
            if slug is None or etype != "heading_sub":
                continue
        ...
        print(f"[INFO] Loaded per-bin Installed Power (MW) for {len(PER_BIN_MW)} bins.")
    else:
...
def add_year_heading(parent_group: QgsLayerTreeGroup, slug: str, label_text: str):
    ...
    if mw_val is not None:
        sub_text = f"Installed Power: {mw_val:,.1f} MW"
