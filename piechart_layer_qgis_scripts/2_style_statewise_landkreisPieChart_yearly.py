...
def add_year_heading(parent_group: QgsLayerTreeGroup, slug: str, label: str, per_bin_mw: float):
    ...
    fmt_s.setBuffer(QgsTextBufferSettings())
...
def main():
    ...
    # Extract PER_BIN_MW as PERIOD values (diff of cumulative totals) — same logic as 1_style
    PER_BIN_MW = {}
    if chart_exists:
        with open(str(CHART_PATH), "r", encoding="utf-8") as f:
            chart = json.load(f)

        # 1) cumulative kW per slug from value_anchor=1 points
        cum_kw = {}
        ...
        # 2) period MW = diff along YEAR_SLUGS
        prev = None
