...
# Purpose:
#   Convert statewise Landkreis pie INPUT POINTS (from step2_1)
#   into actual pie-slice polygons per state, with simple overlap-avoidance.
#
#   Input :
#       pieCharts/statewise_landkreis_pies/de_<state_slug>_landkreis_pies.geojson
#
#   Output:
#       pieCharts/statewise_landkreis_pies/de_<state_slug>_landkreis_pie.geojson
...
def process_one_state(infile: Path):
    state_slug = infile.stem.replace("_landkreis_pies", "")  # de_sachsen_landkreis_pies
            ...
            out_rows.append(
                {
                    ...
                    "name": r.get(
                        "name", r.get("kreis_name", r.get("landkreis_name", ""))
                    ),
                    "energy_type": k,
