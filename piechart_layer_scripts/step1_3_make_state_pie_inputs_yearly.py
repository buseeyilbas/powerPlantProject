...
# ---------- PATHS ----------
INPUT_ROOT = Path(r"C:\Users\jo73vure\Desktop\powerPlantProject\data\geojson\by_state_yearly_three_checks")
            ...
            if vals:
                ...
                for idx, info in enumerate(reversed(yearly_totals)):
                    ...
                    for key in STACK_ORDER:
                        ...
                        seg_ratio = seg_val / max_total if max_total > 0 else 0.0
                        seg_ratio = max(0.0, min(1.0, seg_ratio))
                        seg_width = seg_ratio * MAX_BAR_WIDTH
                        if seg_width <= 0:
                            continue
                ...
                for info in yearly_totals:
                    ...
                    period_total_mw = period_total_kw / 1000.0
                    ...
                    heading_sub = {
                        "year_bin_slug": slug,
                        "year_bin_label": f"Installed Power: {period_total_mw:,.1f} MW",
                        "energy_type": "heading_sub",
                        "energy_kw": 0.0,
                        "total_kw": period_total_kw,
                        "label_anchor": 0,
                        "value_anchor": 0,
