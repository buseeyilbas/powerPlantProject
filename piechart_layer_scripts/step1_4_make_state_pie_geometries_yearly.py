...
#
# UPDATE:
#   - Carries state_abbrev into output polygons.
#   - Keeps radius scaling fully aligned with step1_3 pie size legend.
#   - Preserves fixed centers coming from step1_3.
...
BASE = Path(r"D:\FAU_HiWi\powerPlantProject\data\geojson\pieCharts\state_pies_yearly")
...
def main():
    ...
    if GLOBAL_SIZING and GLOBAL_META.exists():
        meta = json.loads(GLOBAL_META.read_text(encoding="utf-8"))
        global_min = float(meta["min_total_kw"])
        global_max = float(meta["max_total_kw"])
        print(f"[GLOBAL SCALE] Using meta: min={global_min:.2f}, max={global_max:.2f}")
        ...
        for bin_dir in sorted([p for p in BASE.iterdir() if p.is_dir()]):
            ...
            if pts.exists():
                ...
                if len(gdf):
                    totals += list(gdf["total_kw"].astype(float))
    ...
    for bin_dir in sorted([p for p in BASE.iterdir() if p.is_dir()]):
        pts_path = bin_dir / f"de_state_pies_{bin_dir.name}.geojson"
        meta_path = bin_dir / f"state_pie_style_meta_{bin_dir.name}.json"

        if not pts_path.exists() or not meta_path.exists():
            print(f"[SKIP] Missing inputs for bin {bin_dir.name}")
            continue
        ...
        gdf_points = gpd.read_file(pts_path)
        out_geojson = bin_dir / f"de_state_pie_{bin_dir.name}.geojson"

        feature_count = pies_from_points(gdf_points, vmin, vmax, out_geojson)
