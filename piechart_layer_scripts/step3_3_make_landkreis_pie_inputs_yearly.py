...
YEAR_SLUG_ORDER = [slug for slug, *_ in YEAR_BINS]
...
def main():
    ...
    for state_slug, kreis_key in all_kreise:
        ...
        for slug in YEAR_SLUG_ORDER:
            ...
            if pr and pr.get("kreis_name"):
                kreis_name = str(pr.get("kreis_name"))
                break
        if not kreis_name:
            kreis_name = kreis_key
    ...
    global_min = float(min(all_cumulative_totals)) if all_cumulative_totals else 0.0
    global_max = float(max(all_cumulative_totals)) if all_cumulative_totals else 1.0
    if global_max <= global_min:
        global_max = global_min + 1.0

    global_meta = {
        "min_total_kw": global_min,
        "max_total_kw": global_max,
        "r_min_m": R_MIN_M,
        "r_max_m": R_MAX_M,
        "priority_fields": list(PRIORITY.values()),
        "others_field": OTHERS,
        "name_field": "kreis_key",
        "note": "Global sizing stats across ALL bins (cumulative pies).",
    }
