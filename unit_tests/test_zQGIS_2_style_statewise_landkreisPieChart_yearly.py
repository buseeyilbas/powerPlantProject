# Filename: unit_tests/test_zQGIS_2_style_statewise_landkreisPieChart_yearly.py
...
MODULE_NAME = "piechart_layer_qgis_scripts.2_style_statewise_landkreisPieChart_yearly"
...
# -------------------------------------------------------------------
# Fake provider / layer / groups / root / project
# -------------------------------------------------------------------
...
ROOT_DIR = r"C:\Users\jo73vure\Desktop\powerPlantProject\data\geojson\pieCharts\statewise_landkreis_pies_yearly"
CHART_DIR = r"C:\Users\jo73vure\Desktop\powerPlantProject\data\geojson\pieCharts\state_pies_yearly"
YEARLY_CHART_PATH = CHART_DIR + r"\de_yearly_totals_chart.geojson"
GUIDES_PATH = CHART_DIR + r"\de_yearly_totals_chart_guides.geojson"
LEGEND_PATH = CHART_DIR + r"\de_energy_legend_points.geojson"
...
def test_main_replaces_old_group_before_creating_new_one(monkeypatch):
    ...
    # after import a group already exists
    assert "statewise_landkreis_pies (yearly)" in project.root.groups

    old_group = project.root.groups["statewise_landkreis_pies (yearly)"]
    project.root.groups["statewise_landkreis_pies (yearly)"] = old_group

    module.main()

    assert "statewise_landkreis_pies (yearly)" in project.root.groups
    assert project.root.groups["statewise_landkreis_pies (yearly)"] is not old_group


def test_main_happy_path_loads_pies_charts_guides_column_chart_and_heading(monkeypatch):
    pie_pre_1 = ROOT_DIR + r"\de_bayern_landkreis_pie_pre_1990.geojson"
    pie_pre_2 = ROOT_DIR + r"\de_hessen_landkreis_pie_pre_1990.geojson"
    pie_1991 = ROOT_DIR + r"\de_bayern_landkreis_pie_1991_1992.geojson"
    ...
    glob_map = {
        (ROOT_DIR, "de_*_landkreis_pie_pre_1990.geojson"): [pie_pre_1, pie_pre_2],
        (ROOT_DIR, "de_*_landkreis_pie_1991_1992.geojson"): [pie_1991],
    }
    ...
    layer_defs = {
        ...
        pie_pre_1: {"field_names": ["energy_type"]},
        pie_pre_2: {"field_names": ["energy_type"]},
    ...
    parent_group = project.root.findGroup("statewise_landkreis_pies (yearly)")
    ...
    assert "de_bayern_landkreis_pie_pre_1990" in first_names
    assert "de_hessen_landkreis_pie_pre_1990" in first_names
    ...
    assert "de_bayern_landkreis_pie_1991_1992" in second_names
...
def test_main_logs_info_when_no_pie_polygons_found_for_bin(monkeypatch, capsys):
    existing_paths = {ROOT_DIR}
    ...
    captured = capsys.readouterr()
    assert "[INFO] No Landkreis pie polygons found for bin pre_1990" in captured.out


def test_main_warns_when_invalid_pie_layer(monkeypatch, capsys):
    pie_pre = ROOT_DIR + r"\de_bayern_landkreis_pie_pre_1990.geojson"

    existing_paths = {ROOT_DIR}
    glob_map = {
        (ROOT_DIR, "de_*_landkreis_pie_pre_1990.geojson"): [pie_pre],
    }
    layer_defs = {
...
def test_main_handles_period_gw_computation_failure(monkeypatch, capsys):
    ...
    clear_module()
    FakePath.existing_paths = set(existing_paths)
    FakePath.glob_map = {}
    FakePath.file_contents = {}
