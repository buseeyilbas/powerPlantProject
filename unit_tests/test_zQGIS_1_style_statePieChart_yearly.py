# Filename: test_zQGIS_1_style_statePieChart_yearly.py
...
MODULE_NAME = "piechart_layer_qgis_scripts.1_style_statePieChart_yearly"
...
def install_fake_qgis(monkeypatch, project, vector_layer_factory):
    ...
    qgis_core.QgsRuleBasedRenderer = FakeQgsRuleBasedRenderer
...
def test_import_skips_invalid_total_kw_rows(monkeypatch):
    ...
    assert module.PER_BIN_GW == {}
    assert module.CUM_BIN_GW == {}
...
def test_style_year_overview_layer_sets_invisible_marker_and_labels(minimal_import):
    module, _, _ = minimal_import
    lyr = FakeVectorLayer("x", "overview", "ogr")

    module.style_year_overview_layer(lyr)

    renderer = lyr.renderer()
    assert isinstance(renderer, FakeSingleSymbolRenderer)
    assert renderer.symbol.props["name"] == "square"
...
def test_style_center_layer_with_abbrev_builds_offset_rules(minimal_import):
    ...
    assert rules[0].filter_expression == '"state_number" IN (3,5,6)'
    assert rules[1].filter_expression == '"state_number" = 12'
    assert rules[2].filter_expression == '"state_number" IN (1,4,7,8,9,11,13,14,16)'
...
def test_main_builds_groups_and_loads_current_layers(monkeypatch):
    existing_paths = {
        ...
        ROOT_DIR + r"\pre_1990\de_state_pies_pre_1990.geojson",
        ROOT_DIR + r"\1991_1992\de_state_pie_1991_1992.geojson",
        ROOT_DIR + r"\1991_1992\de_state_pies_1991_1992.geojson",
    }
    ...
    assert "de_state_pie_pre_1990" in first_names
    assert "de_state_pies_pre_1990" in first_names
    ...
    assert "de_state_pie_1991_1992" in second_names
    assert "de_state_pies_1991_1992" in second_names
...
def test_main_prints_warning_when_yearly_chart_missing(monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert (
        "[WARN] YEARLY_CHART_PATH not found:" in captured.out
        or "[WARN] Global yearly chart file not found:" in captured.out
    )
