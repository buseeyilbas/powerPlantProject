"""
Unit tests for step18_filter_json_by_state_landkreis_yearly.py
"""
...
from scripts import step18_filter_json_by_state_landkreis_yearly as mod
...
def test_filter_json_by_state_landkreis_yearly_end_to_end(temp_workspace, sample_gadm_l2_geojson, capsys):
    ...
    mod.filter_json_by_state_landkreis_yearly(
        input_folder=str(input_dir),
        output_root=str(output_dir),
        gadm_l2_path=str(sample_gadm_l2_geojson),
    )

    bayern_2020 = output_dir / "Bayern" / "landkreis a" / "2020" / "file1.json"
    bayern_unknown = output_dir / "Bayern" / "landkreis a" / "unknown" / "file1.json"
    thueringen_2021 = output_dir / "Thüringen" / "landkreis b_city" / "2021" / "file1.json"
    ...
    assert "Saved" in out
    assert "Bayern/landkreis a/2020/file1.json" in out
    assert "Bayern/landkreis a/unknown/file1.json" in out
    assert "Thüringen/landkreis b_city/2021/file1.json" in out
    assert "====== SUMMARY ======" in out


def test_filter_json_by_state_landkreis_yearly_custom_date_field(temp_workspace, sample_gadm_l2_geojson):
    input_dir = temp_workspace["input_dir"]
    output_dir = temp_workspace["output_dir"]
    ...
    out_file = output_dir / "Bayern" / "landkreis a" / "2015" / "file1.json"
...
def test_filter_json_by_state_landkreis_yearly_multiple_entries_same_bucket(temp_workspace, sample_gadm_l2_geojson):
    input_dir = temp_workspace["input_dir"]
    output_dir = temp_workspace["output_dir"]
    ...
    out_file = output_dir / "Bayern" / "landkreis a" / "2020" / "file1.json"
    assert out_file.exists()
    assert read_json(out_file) == [entry_1, entry_2]


def test_filter_json_by_state_landkreis_yearly_empty_input_writes_empty_summary(
    temp_workspace,
...
def test_filter_json_by_state_landkreis_yearly_raises_when_l2_missing(temp_workspace):
    input_dir = temp_workspace["input_dir"]
    output_dir = temp_workspace["output_dir"]
