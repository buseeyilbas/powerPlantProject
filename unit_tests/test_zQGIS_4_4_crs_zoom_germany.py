# Filename: unit_tests/test_zQGIS_4_4_crs_zoom_germany.py
...
MODULE_NAME = "piechart_layer_qgis_scripts.4_4_crs_zoom_germany"
...
def test_creates_germany_extent_rectangle_with_expected_coordinates(monkeypatch):
    ...
    extent = created_rectangles[0]

    assert extent.xmin == 3.6
    assert extent.ymin == 47.0
    assert extent.xmax == 16.5
    assert extent.ymax == 55.8
...
def test_transforms_bounding_box_of_original_extent(monkeypatch):
    module, project, canvas, created_rectangles, created_crs, created_transforms = import_module_with_fakes(
        monkeypatch
    )

    transform = created_transforms[0]
    original_extent = created_rectangles[0]

    assert transform.transform_calls == [original_extent]
...
def test_prints_zoom_success_message(monkeypatch, capsys):
    ...
    assert "🔍 Zoomed to Germany." in captured.out