...
for state, landkreis_dict in state_totals.items():
    ...
    for landkreis, total_power in landkreis_dict.items():
        ...
        layer = QgsVectorLayer("Point?crs=EPSG:4326", f"landkreis_pie_{state}_{landkreis}_yearly_statewise", "memory")
        pr = layer.dataProvider()
        pr.addAttributes([QgsField(name, QVariant.Double) for name in ENERGY_TYPES.values()])
        pr.addAttributes([
